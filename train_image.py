...
dataset_text = "D:\KHOALUAN\Video-Caption\data\Flickr8k_text"
dataset_images = "D:\KHOALUAN\Video-Caption\data\Flickr8k_Dataset\Flicker8k_Dataset"
...
def dict_to_list(descriptions):
    all_desc = []
    for key in descriptions.keys():
        [all_desc.append(d) for d in descriptions[key] if d.strip()]
    return all_desc
...
def max_length(descriptions):
    desc_list = dict_to_list(descriptions)
    if not desc_list:
        raise ValueError("No descriptions found in the provided dictionary.")
    # Calculate the maximum length of descriptions
    return max(len(d.split()) for d in desc_list)
    

print("Số lượng train_imgs:", len(train_imgs))
print("train_imgs mẫu:", train_imgs[:5])
print("Số lượng train_descriptions:", len(train_descriptions))
print("Keys train_descriptions:", list(train_descriptions.keys())[:5])
...
model.fit(dataset, epochs=15, steps_per_epoch=steps, verbose=1)
model.save("models2/model_.h5")